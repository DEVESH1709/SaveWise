...
export default function Dashboard({
  ...
  const progressDisplay =
    progress < 1 && progress > 0 ? progress.toFixed(2) : Math.round(progress);
    ...
    <div className="bg-linear-to-r from-indigo-600 via-indigo-500 to-blue-500 rounded-2xl p-6 mb-8 shadow-xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <p className="text-white/80 text-sm flex items-center gap-1">
              <Target className="w-4 h-4" /> Total Targets
            </p>
          </div>
          ...
          <p className="text-white/60 text-sm">
            $
            {totals.targetUSD.toLocaleString("en-US", {
        ...
        <div>
          ...
          <p className="text-white/60 text-sm">
            $
            {totals.savedUSD.toLocaleString("en-US", {
              maximumFractionDigits: 0,
            })}
          </p>
        </div>
